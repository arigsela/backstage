...
PROJECT_NAME = os.getenv("PROJECT_NAME", "${{ values.name }}")

# Ports for each service. These must match the Dockerfile EXPOSE and K8s service definitions.
ORCHESTRATOR_PORT = int(os.getenv("ORCHESTRATOR_PORT", "${{ values.orchestratorPort }}"))
SUB_AGENT_PORT = int(os.getenv("SUB_AGENT_PORT", "${{ values.subAgentPort }}"))
...
SUB_AGENT_URL = os.getenv(
    "SUB_AGENT_URL",
    f"http://${{ values.subAgentName }}:{SUB_AGENT_PORT}"
)
