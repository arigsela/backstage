...
logger = setup_logging("${{ values.subAgentName }}.server")
...
AGENT_CARD = AgentCard(
    name="${{ values.subAgentDisplayName }}",
    description="${{ values.subAgentGoal }}",
...
def create_app() -> FastAPI:
    ...
    application = FastAPI(
        title=f"{PROJECT_NAME} - ${{ values.subAgentDisplayName }}",
        version="1.0.0",
    )
    ...
    async def health():
        return {"status": "healthy", "service": "${{ values.subAgentName }}"}
    ...
    from ${{ values.subAgentName }}.executor import SubAgentExecutor
